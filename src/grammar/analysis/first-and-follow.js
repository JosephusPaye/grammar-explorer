...
function firstOfElement(element, firstCache) {
  ...
  element.allProductions().forEach((production) => {
    ...
    if (
      production.elements.length === 1 &&
      production.elements[0] &&
      production.elements[0].isEpsilon()
    ) {
...
export function addFollows(grammar) {
  const grammarProductions = Object.values(grammar)
    .map((nonTerminal) => {
      return nonTerminal.allProductions().map((production) => {
        return {
          nonTerminal,
...
function addFollow(nonTerminal, grammarProductions, pass) {
  ...
  if (pass === 1) {
    grammarProductions.forEach((production) => {
      ...
      if (location.hasElementsAfter) {
        nonTerminal.followSet.addAll(
          firstOfListFromComputed(
            production.elements.slice(location.index + 1)
          ).except(epsilon)
        );
      }
    });
    ...
    grammarProductions.forEach((production) => {
      ...
      if (
        !location.hasElementsAfter ||
        producesEpsilon(production.elements.slice(location.index + 1))
      ) {
